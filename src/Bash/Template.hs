...
read_values :: Bool -> Text
read_values rand = [qc|
declare -A VALUES

{if rand then second else ""}

# usage: read_values <path> <value>;
function read_values() \{
    local FIELD_PATH=$(echo $1 | sed 's/\[/\(/g' - | sed 's/\]/\)/g' - )
    VALUES["$FIELD_PATH"]="{if rand then value else "$2"}"
}
|]
  where
    value = "second 'od -vAn -N2 -tu2 < /dev/random' $2" :: Text
